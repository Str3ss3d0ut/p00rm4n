...
st.set_page_config(page_title="Alpha Screener v12 (Ultimate)", layout="wide")
st.title("🚀 Personal Stock Alpha Screener v12 (The Full Restoration)")
...
# Default Universe
default_tickers = (
    "MMM, AOS, ABBV, AES, AFL, APD, AKAM, ALGN, ALLE, LNT, ALL, MO, AMCR, AEE, AEP, AIG, AMT, AWK, AME, AMGN, ADI, APA, AAPL, AMAT, APTV, ACGL, ADM, T, ATO, AZO, AVB, AVY, BKR, BALL, BAC, BDX, BRK-B, BIIB, BLK, BK, BA, BMY, BF-B, BLDR, BG, CPT, CPB, CAH, CCL, CARR, CAT, CBOE, COR, CNP, CF, CHTR, CVX, CB, CHD, CIEN, CI, CINF, CTAS, CFG, CLX, CME, CMS, KO, CL, CMCSA, FIX, CAG, COP, ED, STZ, COO, GLW, CPAY, CTVA, COST, CTRA, CRH, CCI, CSX, CMI, CVS, DRI, DVA, DECK, DE, DAL, DVN, FANG, DLR, DG, DLTR, D, DOV, DOW, DHI, DTE, DUK, DD, ETN, ECL, EIX, EA, EME, ETR, EOG, EQT, EQIX, EQR, ERIE, ESS, EG, EVRG, ES, EXC, EXR, XOM, FAST, FRT, FDX, FITB, FE, F, FTNT, FTV, BEN, FCX, GRMN, GE, GEV, GNRC, GD, GIS, GM, GPC, GILD, GL, GS, HAL, HIG, HAS, HCA, DOC, HSIC, HSY, HLT, HOLX, HD, HON, HRL, HST, HWM, HUBB, HII, IEX, ITW, INCY, IR, IBKR, IFF, IP, INVH, IRM, JBHT, JBL, JNJ, JCI, JPM, KVUE, KDP, KEY, KEYS, KMB, KIM, KMI, KHC, KR, LHX, LH, LRCX, LEN, LII, LLY, LIN, LYV, LMT, L, LOW, LYB, MTB, MPC, MAR, MRSH, MLM, MAS, MA, MKC, MCD, MCK, MDT, MRK, MET, MTD, MCHP, MU, MAA, TAP, MDLZ, MPWR, MNST, MOS, MSI, NEE, NKE, NI, NDSN, NSC, NTRS, NOC, NCLH, NRG, NUE, NVDA, NVR, NXPI, ORLY, OXY, ON, OKE, OTIS, PCAR, PKG, PH, PNR, PEP, PFE, PCG, PM, PSX, PNW, PNC, POOL, PPG, PPL, PFG, PG, PGR, PLD, PEG, PSA, PHM, PWR, DGX, Q, RL, RJF, RTX, O, REG, REGN, RF, RSG, ROST, RCL, SNDK, SBAC, SLB, STX, SRE, SHW, SPG, SJM, SW, SNA, SO, LUV, SWK, SBUX, STT, STLD, STE, SYK, SYY, TMUS, TPR, TRGP, TGT, TEL, TDY, TER, TXN, TPL, TXT, TJX, TKO, TSCO, TT, TRV, TFC, TSN, USB, UDR, ULTA, UNP, UAL, UPS, URI, UHS, VLO, VTR, VZ, VRTX, VTRS, VICI, V, VST, VMC, WRB, GWW, WAB, WMT, WBD, WM, WEC, WELL, WDC, WY, WSM, WMB, XEL, YUM, ZBRA, ZBH, ZTS"
)
...
def send_email_report(df_top10, sender, password, recipient, host, port):
    try:
        ...
        <html><body style="font-family: Arial;">
            ...
            {html_table}<br><p>Generated by Katie (v12)</p>
        </body></html>
        """
...
with tab1:
    ...
    auto_email = st.checkbox("📧 Auto-email Top 10?", value=False)
        ...
        if market_healthy:
            ...
            if data.empty:
                ...
                if alpha_data:
                    ...
                    top_pick = df.iloc[0]
                    if "HOT" in top_pick['Signal']:
                        st.balloons()
                        st.success(f"🏆 TOP PICK: **{top_pick['Symbol']}** | Alloc: {top_pick['Alloc %']:.1f}%")
                    
                    try:
                        st.dataframe(df.style.format({"Price":"${:.2f}", "Score":"{:.1f}", "Alloc %":"{:.1f}%", "Stop Price":"${:.2f}", "RS vs SPY":"{:.2f}%", "Vol Ratio":"{:.2f}", "ADX":"{:.1f}"}).background_gradient(subset=["Confidence"], cmap="Greens"))
                    except:
                        st.dataframe(df)
                    
                    if auto_email and email_sender:
                        success, msg = send_email_report(df.head(10), email_sender, email_password, email_recipient, email_host, email_port)
